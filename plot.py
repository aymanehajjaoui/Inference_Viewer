...
class MainWindow(QMainWindow):
    ...
    def _setup_ui(self):
        ...
        # Final vertical splitter (main + bottom)
    ...
    def restart_receiver(self, ch):
        ...
        thread.start()
        print(f"[CH{ch+1}] (Re)started on {ip}:{port}")
        ...
        while True:
            try:
                with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
                    ...
                    s.bind((ip, port))
                    s.listen(1)
                    print(f"[CH{ch+1}] Listening on {ip}:{port}...")
                    conn, addr = s.accept()
                    print(f"[CH{ch+1}] Connected to {addr}")
                    with conn:
                        while True:
                            data = conn.recv(4)
                            if not data:
